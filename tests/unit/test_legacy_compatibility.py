...
class TestLegacyErrorMode:
    ...
    def test_all_error_types_legacy_mode(self):
        ...
        error_tests = [
            (SessionError, "Session error", "Session error occurred", {"session_state": "test"}),
            (ConnectionError, "Connection failed", "Connection failed to server", {"url": "http://test"}),
            (XMLParsingError, "XML parse error", "Failed to parse XML", {}),
            (RateLimitError, "Rate limit exceeded", None, {"retry_after": 60}),  # Special case - no message arg
            (NoDataError, "No data found", None, {"rssd_id": "12345"}),  # Special case - no message arg
        ]
