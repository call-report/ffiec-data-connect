...
from ffiec_data_connect.exceptions import ValidationError, NoDataError, ConnectionError
...
class TestClientManagement:
    ...
    def test_client_factory_ffiec_connection(self):
        ...
        with patch('ffiec_data_connect.methods._return_client_session') as mock_return_client:
            mock_client = Mock()
...
class TestCollectReportingPeriods:
    ...
    def test_collect_reporting_periods_call_series(self, mock_client_factory):
        ...
        mock_client.service.RetrieveReportingPeriods.assert_called_once_with(dataSeries="Call")
...
class TestCollectData:
    ...
    def test_collect_data_ubpr_series(self, mock_client_factory, mock_process_xml):
        ...
        mock_client.service.RetrieveUBPRXBRLFacsimile.assert_called_once_with(
            fiIDType="ID_RSSD",
            fiID=123456,
            reportingPeriodEndDate="3/31/2023"
        )
    ...
    def test_collect_data_validation(self, mock_client_factory):
        ...
        error_message = str(exc_info.value)
        assert "output_type" in error_message.lower() or "invalid" in error_message.lower()
...
class TestCollectFilersSinceDate:
    ...
    def test_collect_filers_since_date_validation(self, mock_client_factory):
        ...
        with patch('ffiec_data_connect.methods._is_valid_date_or_quarter') as mock_valid:
...
class TestCollectFilersSubmissionDateTime:
    ...
    @patch('ffiec_data_connect.methods._client_factory')
    def test_collect_filers_submission_date_time_python_format(self, mock_client_factory):
...
class TestCollectFilersOnReportingPeriod:
    ...
    @patch('ffiec_data_connect.methods._client_factory')
    def test_collect_filers_on_reporting_period(self, mock_client_factory, mock_normalize):
        """Test basic functionality."""
        mock_raw_data = [
            {"some": "raw_data_1"},
            {"some": "raw_data_2"}
...
class TestErrorHandling:
    ...
    def test_quarter_conversion_edge_cases(self):
        ...
        result = _convert_quarter_to_date("5Q2023")  # Invalid quarter - doesn't match regex [1-4]
        assert result is None
        
        result = _convert_quarter_to_date("0Q2023")  # Invalid quarter - doesn't match regex [1-4]
        assert result is None
