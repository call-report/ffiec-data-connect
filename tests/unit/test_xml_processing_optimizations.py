...
<xbrl xmlns="http://www.xbrl.org/2003/instance">
    ...
    <uc:UBPR4107>12.50</uc:UBPR4107>
        ...
        with patch("ffiec_data_connect.xbrl_processor.xmltodict.parse") as mock_parse:
            mock_parse.return_value = {
                "xbrl": {"cc:RCON0010": "1000000", "uc:UBPR4107": "12.50"}
            }
...
<xbrl xmlns="http://www.xbrl.org/2003/instance">
    ...
    <cc:RCON0071>500000</cc:RCON0071>
        ...
        with patch("ffiec_data_connect.xbrl_processor.xmltodict.parse") as mock_parse:
            mock_parse.return_value = {
                "xbrl": {"cc:RCON0010": "1000000", "cc:RCON0071": "500000"}
            }
...
<invalid_root>
    <cc:RCON0010>1000000</cc:RCON0010>
        ...
        with patch("ffiec_data_connect.xbrl_processor.xmltodict.parse") as mock_parse:
            mock_parse.return_value = {"invalid_root": {"cc:RCON0010": "1000000"}}
    ...
    <cc:ITEM4>true</cc:ITEM4>
        ...
        with patch("ffiec_data_connect.xbrl_processor.xmltodict.parse") as mock_parse:
            ...
            with patch(
                ...
                assert (
                    np.isnan(str_item["int_data"])
                    and np.isnan(str_item["float_data"])
                    and np.isnan(str_item["bool_data"])
                )

                int_item = next(item for item in result if item["mdrm"] == "ITEM2")
                assert int_item["int_data"] == 123
                assert (
                    int_item["str_data"] is None
                    and np.isnan(int_item["float_data"])
                    and np.isnan(int_item["bool_data"])
                )

                float_item = next(item for item in result if item["mdrm"] == "ITEM3")
                assert float_item["float_data"] == 12.5
                assert (
                    float_item["str_data"] is None
                    and np.isnan(float_item["int_data"])
                    and np.isnan(float_item["bool_data"])
                )

                bool_item = next(item for item in result if item["mdrm"] == "ITEM4")
                assert (
                    bool(bool_item["bool_data"]) is True
                )  # Convert numpy bool to Python bool
                assert (
                    bool_item["str_data"] is None
                    and np.isnan(bool_item["int_data"])
                    and np.isnan(bool_item["float_data"])
                )
