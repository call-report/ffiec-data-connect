...
class MemoryTestBase:
    ...
    def monitor_memory_over_time(self, func, duration_seconds: float = 1.0, interval: float = 0.1):
...
class TestCredentialsMemoryLeaks(MemoryTestBase):
    ...
    def test_credential_object_cleanup(self):
        ...
        assert alive <= max_alive, f"{alive} out of {total} credential objects not garbage collected"
    ...
    def test_credential_string_interning(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(
            lambda: [WebserviceCredentials("same_user", "same_pass") for _ in range(1000)]
        )
    ...
    def test_credential_memory_after_gc(self):
        ...
        assert memory_retained < memory_used * 0.1, f"Too much memory retained: {memory_retained} bytes"
    
    def test_environment_credential_caching_memory(self):
        """Test memory usage of environment credential caching."""
        with patch.dict('os.environ', {'FFIEC_USERNAME': 'env_user', 'FFIEC_PASSWORD': 'env_pass'}):
            def create_env_credentials():
                return [WebserviceCredentials() for _ in range(100)]
            
            result, peak, growth, rss_growth = self.measure_memory_usage(create_env_credentials)
            ...
            assert growth < max_expected_growth, f"Inefficient env credential memory: {growth} bytes"
...
class TestFFIECConnectionMemoryLeaks(MemoryTestBase):
    ...
    def test_connection_cleanup_on_close(self):
        ...
        assert growth < max_expected_growth, f"Connection cleanup leaked memory: {growth} bytes"
    ...
    def test_session_caching_memory_efficiency(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(create_connections_with_sessions)
        ...
        assert growth < max_expected_growth, f"Session caching memory inefficient: {growth} bytes"
    ...
    def test_proxy_configuration_memory_stability(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(reconfigure_proxy_many_times)
        
        conn.close()
        
        # Proxy reconfigurations shouldn't leak memory
        max_expected_growth = 100 * 2000  # 200KB allowance (proxy configs + sessions)
        assert growth < max_expected_growth, f"Proxy reconfiguration leaked memory: {growth} bytes"
    ...
    def test_connection_garbage_collection(self):
        ...
        assert alive <= max_alive, f"{alive} out of {total} connections not garbage collected"
    ...
    def test_session_regeneration_memory_leak(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(force_session_regenerations)
        
        conn.close()
        
        # Session regenerations shouldn't accumulate memory
        max_expected_growth = 50 * 2000  # 100KB allowance
        assert growth < max_expected_growth, f"Session regeneration leaked memory: {growth} bytes"
...
class TestAsyncCompatibleClientMemoryLeaks(MemoryTestBase):
    ...
    def test_client_data_collection_memory(self, mock_collect_data):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(perform_data_collections)
        
        # Data collection should be memory efficient
        max_expected_growth = 100 * 5000  # 500KB allowance
        assert growth < max_expected_growth, f"Data collection memory inefficient: {growth} bytes"
    ...
    def test_connection_cache_memory_management(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(test_connection_caching)
        
        # Connection caching should cleanup properly
        max_expected_growth = 20 * 3000  # 60KB allowance
        assert growth < max_expected_growth, f"Connection cache memory leak: {growth} bytes"
    ...
    def test_rate_limiter_memory_stability(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(stress_rate_limiter)
        
        # Rate limiter should use constant memory
        max_expected_growth = 10000  # 10KB allowance
        assert growth < max_expected_growth, f"Rate limiter memory growth: {growth} bytes"
    ...
    def test_client_garbage_collection(self):
        ...
        assert alive <= max_alive, f"{alive} out of {total} clients not garbage collected"
    ...
    def test_parallel_processing_memory(self, mock_collect_data):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(parallel_data_collection)
        
        # Parallel processing should manage memory efficiently
        max_expected_growth = 50 * 8000  # 400KB allowance
        assert growth < max_expected_growth, f"Parallel processing memory inefficient: {growth} bytes"
...
class TestMethodsMemoryLeaks(MemoryTestBase):
    ...
    def test_soap_client_caching_memory(self, mock_client_factory):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(create_many_cached_clients)
        ...
        assert growth < max_expected_growth, f"SOAP client caching inefficient: {growth} bytes"
    ...
    def test_validation_function_memory_stability(self):
        ...
        from ffiec_data_connect.methods import (
            _validate_rssd_id, _date_format_validator, _output_type_validator
        )
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(stress_test_validators)
        
        # Validation functions should use minimal memory
        max_expected_growth = 50000  # 50KB allowance
        assert growth < max_expected_growth, f"Validation functions memory growth: {growth} bytes"
    ...
    def test_date_utility_memory_efficiency(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(stress_test_date_utilities)
        
        # Date utilities should be memory efficient
        max_expected_growth = 500 * 500  # 250KB allowance
        assert growth < max_expected_growth, f"Date utilities memory inefficient: {growth} bytes"
...
class TestIntegrationMemoryLeaks(MemoryTestBase):
    ...
    def test_full_workflow_memory_stability(self, mock_collect_data):
        ...
        mock_collect_data.return_value = [{"test": "data", "size": "x" * 1000}]  # 1KB per result
        ...
        def simulate_full_workflow():
            ...
            for round_num in range(5):
                ...
                round_results = client.collect_data_parallel(f"2023-0{round_num+1}-31", rssd_ids)
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(simulate_full_workflow)
        ...
        assert growth < max_expected_growth, f"Full workflow memory inefficient: {growth} bytes"
    ...
    def test_long_running_session_memory(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(simulate_long_running_session)
    ...
    def test_exception_handling_memory_cleanup(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(test_with_exceptions)
        
        # Exception handling shouldn't leak memory
        max_expected_growth = 50 * 4000  # 200KB allowance (exceptions have stack trace overhead)
        assert growth < max_expected_growth, f"Exception handling leaked memory: {growth} bytes"
...
class TestMemoryPressureScenarios(MemoryTestBase):
    ...
    def test_high_volume_object_creation(self):
        ...
        result, peak, growth, rss_growth = self.measure_memory_usage(create_high_volume_objects)
        
        # High volume creation should manage memory reasonably
        max_expected_growth = 1000 * 10000  # 10MB allowance
        assert growth < max_expected_growth, f"High volume creation memory issue: {growth} bytes"
    ...
    def test_memory_cleanup_under_gc_pressure(self):
        ...
        assert growth < max_expected_growth, f"GC pressure handling issue: {growth} bytes"
