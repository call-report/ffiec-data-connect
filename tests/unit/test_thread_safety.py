...
class TestCredentialsThreadSafety(ThreadSafetyTestBase):
    ...
    def test_concurrent_credential_creation(self):
        ...
        results = self.run_concurrent_test(
            create_credentials, num_threads=15, iterations_per_thread=5
        )

        # Should have no errors
        assert (
            len(results["errors"]) == 0
        ), f"Credential creation errors: {results['errors']}"
        ...
        def attempt_modification(thread_id, iteration):
            try:
                ...
                try:
                    ...
                    modification_attempts.put(
                        ("password", thread_id, iteration, "SUCCESS")
                    )
                    return "SECURITY_BREACH"
                except Exception:
                    modification_attempts.put(
                        ("password", thread_id, iteration, "BLOCKED")
                    )
                    return "MODIFICATION_BLOCKED"

        results = self.run_concurrent_test(
            attempt_modification, num_threads=20, iterations_per_thread=10
        )

        # All modification attempts should be blocked
        successful_modifications = [
            r for r in results["results"] if r[2] == "SECURITY_BREACH"
        ]
        assert (
            len(successful_modifications) == 0
        ), "Security breach: credentials were modified!"
    ...
    def test_concurrent_environment_credential_access(self):
        ...
        results = self.run_concurrent_test(
            create_env_credentials, num_threads=25, iterations_per_thread=8
        )
...
class TestFFIECConnectionThreadSafety(ThreadSafetyTestBase):
    ...
    def test_concurrent_connection_creation(self):
        ...
        results = self.run_concurrent_test(
            create_connection, num_threads=15, iterations_per_thread=5
        )
    ...
    def test_concurrent_session_access(self):
        ...
        results = self.run_concurrent_test(
            access_session, num_threads=20, iterations_per_thread=10
        )
        ...
        ), f"Expected 1 unique session, got {len(unique_session_ids)}"
    ...
    def test_concurrent_proxy_configuration(self):
        ...
        results = self.run_concurrent_test(
            configure_proxy, num_threads=10, iterations_per_thread=3
        )

        # Should complete without deadlocks or crashes
        assert (
            results["total_time"] < 10.0
        ), "Proxy configuration took too long (possible deadlock)"
    ...
    def test_concurrent_session_regeneration(self):
        ...
        results = self.run_concurrent_test(
            trigger_session_regeneration, num_threads=10, iterations_per_thread=5
        )
    ...
    def test_connection_cleanup_thread_safety(self):
        ...
        results = self.run_concurrent_test(
            create_and_cleanup_connection, num_threads=15, iterations_per_thread=8
        )
...
class TestAsyncCompatibleClientThreadSafety(ThreadSafetyTestBase):
    ...
    def test_concurrent_client_creation(self):
        ...
        def create_client(thread_id, iteration):
            ...
            client = AsyncCompatibleClient(creds, max_concurrent=3, rate_limit=20)
        ...
        results = self.run_concurrent_test(
            create_client, num_threads=12, iterations_per_thread=5
        )
    ...
    def test_concurrent_connection_caching(self):
        ...
        results = self.run_concurrent_test(
            get_cached_connection, num_threads=8, iterations_per_thread=10
        )
        ...
        for thread_id, conn_ids in connection_ids.items():
            ...
            assert (
                len(unique_ids) == 1
            ), f"Thread {thread_id} got multiple connections: {unique_ids}"
    ...
    def test_concurrent_data_collection(self, mock_collect_data):
        ...
        results = self.run_concurrent_test(
            collect_data_concurrently, num_threads=10, iterations_per_thread=8
        )
    ...
    def test_rate_limiter_thread_safety(self):
        ...
        results = self.run_concurrent_test(
            make_rate_limited_call, num_threads=15, iterations_per_thread=4
        )
        ...
        delayed_calls = [
            call for call in sorted_calls if call[4] > 0.005
        ]  # 5ms tolerance
        assert (
            len(delayed_calls) >= len(sorted_calls) * 0.7
        )  # At least 70% should be delayed
    ...
    def test_client_cleanup_thread_safety(self):
        ...
        results = self.run_concurrent_test(
            create_and_cleanup_client, num_threads=12, iterations_per_thread=5
        )
...
class TestMethodsThreadSafety(ThreadSafetyTestBase):
    ...
    def test_concurrent_input_validation(self):
        ...
        results = self.run_concurrent_test(
            run_validation_tests, num_threads=20, iterations_per_thread=10
        )
    ...
    def test_concurrent_date_utilities(self):
        ...
        results = self.run_concurrent_test(
            run_date_operations, num_threads=25, iterations_per_thread=15
        )
...
class TestConcurrentResourceManagement(ThreadSafetyTestBase):
    ...
    def test_memory_pressure_under_concurrency(self):
        ...
        def create_heavy_objects(thread_id, iteration):
            ...
            creds = WebserviceCredentials(
                f"user_{thread_id}_{iteration}", f"pass_{thread_id}_{iteration}"
            )
            conn = FFIECConnection()
        ...
        results = self.run_concurrent_test(
            create_heavy_objects, num_threads=8, iterations_per_thread=10
        )
    ...
    def test_exception_safety_under_concurrency(self):
        ...
        def potentially_failing_operation(thread_id, iteration):
            try:
                ...
                if (thread_id + iteration) % 7 == 0:  # Fail predictably
                    raise ValueError(
                        f"Simulated error for thread {thread_id}, iteration {iteration}"
                    )
        ...
        results = self.run_concurrent_test(
            potentially_failing_operation, num_threads=15, iterations_per_thread=10
        )
...
class TestStressConditions(ThreadSafetyTestBase):
    ...
    def test_high_concurrency_stress(self):
        ...
        assert (
            len(results["errors"]) == 0
        ), f"Errors under high concurrency: {results['errors'][:5]}"
        assert (
            results["success_rate"] >= 0.95
        )  # Allow 5% failure rate under extreme stress

        # Should complete in reasonable time
        assert (
            results["total_time"] < 45.0
        ), f"High concurrency test too slow: {results['total_time']}s"
    ...
    def test_rapid_creation_destruction(self):
        ...
        def create_destroy_cycle(thread_id, iteration):
            ...
            for i in range(5):
                creds = WebserviceCredentials(
                    f"user_{thread_id}_{iteration}_{i}", f"pass_{i}"
                )
                conn = FFIECConnection()
        ...
        results = self.run_concurrent_test(
            create_destroy_cycle, num_threads=20, iterations_per_thread=8
        )
