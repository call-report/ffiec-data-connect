...
def collect_reporting_periods_enhanced(
    ...
    try:
        ...
        logger.debug(f"Retrieved and validated {len(raw_periods)} reporting periods via REST API")
        
        # Validate that we received properly formatted data
        validation_report = DataNormalizer.validate_pydantic_compatibility(
            raw_periods, 
            "RetrieveUBPRReportingPeriods" if series.lower() == "ubpr" else "RetrieveReportingPeriods"
        )
        
        if not validation_report["compatible"]:
            logger.warning(f"Schema validation warnings: {validation_report['warnings']}")
        ...
        raise_exception(
            ConnectionError,
            f"Failed to retrieve reporting periods via REST API: {e}"
        )
...
def collect_filers_on_reporting_period_enhanced(
    ...
    """
    logger.debug(f"collect_filers_on_reporting_period_enhanced called for period={reporting_period}")
    ...
    try:
        ...
        logger.error(f"REST API call failed in collect_filers_on_reporting_period_enhanced: {e}")
        raise_exception(
            ConnectionError,
            f"Failed to retrieve filers via REST API: {e}"
        )
...
def collect_filers_since_date_enhanced(
    ...
    """
    logger.debug(f"collect_filers_since_date_enhanced called for period={reporting_period}, since={since_date}")
    ...
    try:
        # Create protocol adapter and call REST endpoint
        adapter = create_protocol_adapter(creds, session)
        rssd_ids = adapter.retrieve_filers_since_date(ffiec_reporting_period, ffiec_since_date)
        ...
        raise_exception(
            ConnectionError,
            f"Failed to retrieve filers since date via REST API: {e}"
        )
...
def collect_filers_submission_date_time_enhanced(
    ...
    """
    logger.debug(f"collect_filers_submission_date_time_enhanced called for period={reporting_period}, since={since_date}")
    ...
    try:
        # Create protocol adapter and call REST endpoint
        adapter = create_protocol_adapter(creds, session)
        submissions = adapter.retrieve_filers_submission_datetime(ffiec_reporting_period, ffiec_since_date)
        ...
        for submission in submissions:
            processed_sub = {
                'id_rssd': str(submission.get('ID_RSSD', '')),
                'datetime': _format_datetime_for_output(
                    submission.get('DateTime', ''), 
                    date_output_format
                )
            }
        ...
        logger.error(f"REST API call failed in collect_filers_submission_date_time_enhanced: {e}")
        raise_exception(
            ConnectionError,
