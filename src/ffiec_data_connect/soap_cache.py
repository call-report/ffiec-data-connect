...
class SOAPClientConfig:
    ...
    def from_credentials_and_session(
        ...
        if hasattr(session, 'use_proxy') and session.use_proxy:
            proxy_config = f"{session.proxy_protocol}://{session.proxy_host}:{session.proxy_port}"
...
class SOAPClientCache:
    ...
    def _evict_lru(self) -> None:
        ...
        try:
            ...
            if hasattr(client.transport, 'session') and hasattr(client.transport.session, 'close'):
